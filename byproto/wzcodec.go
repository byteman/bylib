...
反加1
 */
func (WzProtoCodec) Decode(conn net.Conn) (byudp.Message, error) {
	//读取出数据包
	var buf [1024]byte
	n,err:=conn.Read(buf[:])
	if err!=nil{
 ...
 */
func (WzProtoCodec) Encode(msg byudp.Message) ([]byte, error) {
...
func DeserializeMyMessage(data []byte) (message byudp.Message, err error) {
	if data == nil {
		return nil, byudp.ErrNilData
	}
