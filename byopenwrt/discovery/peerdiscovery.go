...
func (m *MultiCaster)Send(payload []byte)error{
	ifaces, err := net.Interfaces()
	if err != nil {
		bylog.Error("get interface err=%v",err)
		return err
	}
	if m.tx!=nil{
			//bylog.Debug("interface = %v",ifaces)
			// write to multicast
			dst := &net.UDPAddr{IP: m.s.multicastAddressNumbers, Port: m.s.portNum}
			for i := range ifaces {
				if m.s.IPVersion == IPv4 {
					p24 := m.tx.(*ipv4.PacketConn)
					if errMulticast := p24.SetMulticastInterface(&ifaces[i]); errMulticast != nil {
						//bylog.Error("SetMulticastInterface err=%v",errMulticast)
						continue
					}
...
func (m *MultiCaster)Listen(settings ...Settings) (err error) {
	s := Settings{}
	if len(settings) > 0 {
		s = settings[0]
	}
